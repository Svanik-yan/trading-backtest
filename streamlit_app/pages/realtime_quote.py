    ...
    if quote_data is not None:
        ...
        with col1:
            ...
            st.metric(
                ...
                delta=f"{float(quote_data['price'].values[0]) - float(quote_data['pre_close'].values[0]):.2f}"
            )
            ...
            fig.add_trace(go.Indicator(
                ...
                delta={'reference': float(quote_data['pre_close'].values[0])},
                title={'text': "价格变动"}
            ))
            st.plotly_chart(fig)
        ...
        with col2:
            # 显示交易信息
            st.metric("成交量", f"{int(quote_data['volume'].values[0]):,}")
            st.metric("成交额", f"¥{float(quote_data['amount'].values[0]):,.2f}")
        ...
        # 显示详细数据表格
        st.subheader("交易详情")
        display_cols = ['code', 'name', 'price', 'bid', 'ask', 
                       'volume', 'amount', 'time']
        st.dataframe(
